...
path_result_res1 = linker_path_to_result_file(
    ["euler_hawkes", f"{STR_CONFIG}_res_1", "data_together", "results_together.csv"])
path_plot_res1 = linker_path_to_result_file(["euler_hawkes", f"{STR_CONFIG}_res_1", "plot", ""])
...
path_result_res2 = linker_path_to_result_file(
    ["euler_hawkes", f"{STR_CONFIG}_res_2", "data_together", "results_together.csv"])
path_plot_res2 = linker_path_to_result_file(["euler_hawkes", f"{STR_CONFIG}_res_2", "plot", ""])

estim_hp2 = Estim_hawkes.from_csv(path_result_res2)
kernels_to_plot_hitdep = list_of_kernels_for_hitdep, times_estimation
...
#                     kernels_to_plot=kernels_to_plot_itideep, draw_all_kern=ALL_KERNELS_DRAWN)

# kernels for hitdep:
print("\n~~~~~~~~~~~~~~~~~~~~~~~~Image together of ITDEP~~~~~~~~~~~~~~~~~~~~~~~~")
estim_total = estim_hp.append(estim_hp2.df)
path_plot_res3 = linker_path_to_result_file(["euler_hawkes", f"{STR_CONFIG}_res_3", "plot", ""])
kernels_to_plot_hitdep = list_of_kernels_for_hitdep, times_estimation
